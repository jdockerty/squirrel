...
// `sqrl-client` with no args should exit with a non-zero code.
#[test]
fn client_cli_no_args() {
    let temp_dir = TempDir::new().unwrap();
    let mut cmd = Command::cargo_bin("sqrl-client").unwrap();
...
// `sqrl-client -V` should print the version
#[test]
...
// `sqrl-server -V` should print the version
#[test]
...
fn cli_log_configuration() {
    ...
    let mut cmd = Command::cargo_bin("sqrl-server").unwrap();
    let mut child = cmd
        .args(["--engine", "sqrl", "--addr", "127.0.0.1:4001"])
        .current_dir(&temp_dir)
...
fn cli_wrong_engine() {
    // sled first, sqrl second
    {
        ...
        let mut cmd = Command::cargo_bin("sqrl-server").unwrap();
        cmd.args(["--engine", "sqrl", "--addr", "127.0.0.1:4003"])
            .current_dir(&temp_dir)
            .assert()
            .failure();
    }

    // sqrl first, sled second
    {
        let temp_dir = TempDir::new().unwrap();
        let mut cmd = Command::cargo_bin("sqrl-server").unwrap();
        let mut child = cmd
            .args(["--engine", "sqrl", "--addr", "127.0.0.1:4002"])
            .current_dir(&temp_dir)
            .spawn()
            .unwrap();
        thread::sleep(Duration::from_secs(1));
...
fn cli_access_server(engine: &str, addr: &str) {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut server = Command::cargo_bin("sqrl-server").unwrap();
...
#[test]
fn cli_access_server_sqrl_engine() {
    cli_access_server("sqrl", "127.0.0.1:4004");
}
