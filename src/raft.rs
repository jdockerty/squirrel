...
pub type NodeId = u64;
pub type LogStore = crate::raft_log::LogStore<TypeConfig>;
...
impl RaftSnapshotBuilder<TypeConfig> for Arc<StateMachineStore> {
    ...
    async fn build_snapshot(&mut self) -> Result<Snapshot<TypeConfig>, StorageError<NodeId>> {
...

        {
            ...
            data = serde_json::to_vec(&state_machine.data_state)
            ...
            match entry.payload {
                ...
                EntryPayload::Normal(ref req) => match req {
                    Request::Set { key, value } => {
                        sm.data.set(key.clone(), value.clone()).await.unwrap();
                        res.push(Response {
